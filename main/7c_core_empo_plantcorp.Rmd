...
melt_abun$sample_type <- meta$sample_type[match(melt_abun$sequencing_id, meta$sequencing_id)]

#trophs = c("PrimaryProducer", "Environmental", "Consumer")
habitats = c("Terrestrial", "Riverine", "Marine")
empo_cats <- unique(meta$empo_3)

# just plantal corpus for now
plant_corp_table <- melt_abun[which(melt_abun$empo3=="Plant corpus" & melt_abun$habitat %in% habitats),]


plant_corp_list <- list()

for (a_hab in habitats) {
  sub_df <- plant_corp_table[which(plant_corp_table$habitat==a_hab),]
  plant_corp_list[[a_hab]] <- sub_df
}



num_samps <- sapply(plant_corp_list, function(x) length(unique(x$sequencing_id)))


prevs <- list()

for (i in 1:length(plant_corp_list)) {
  df <- as.data.frame(plant_corp_list[[i]])
...
# terrestrial 
plant_corp_plots <- lapply(prevs, prev_plot)

for (i in 1:length(habitats)) {
  pl <- plant_corp_plots[[i]] + labs(title = habitats[i])
  plant_corp_plots[[i]] <- pl
}

library(ggpubr)
pcp <- ggarrange(plant_corp_plots[[1]], plant_corp_plots[[2]], plant_corp_plots[[3]], nrow=1)

pcp <- annotate_figure(pcp, top = text_grob("Plant Corpus", color = "black", face = "bold", size = 14))

ggsave("../figures/plant_corpus_prevalence.png", width=17, height=5)
