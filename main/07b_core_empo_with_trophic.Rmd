...
for (a_hab in habitats) {
  melt_list[[a_hab]] = lapply(trophs,
                              make_melt_tables,
                              hab = a_hab,
                              melt_table = anim_corp_table)
}

names(melt_list[["Terrestrial"]]) <- trophs
names(melt_list[["Riverine"]]) <- trophs
names(melt_list[["Marine"]]) <- trophs

saveRDS(melt_list, "../intermediates/melted_list_of_samples_and_reads_for_core.rds")
...
prev_calc <- function(df, denom) {
  ...
  df <- as.data.frame(df)
  df <- df %>% group_by(ASV) %>% summarise(uniq_id = n_distinct(sequencing_id),
                                           prev = n_distinct(sequencing_id)/denom,
                                           den = denom)
  return(df)
}
...
ggsave("../figures/empo3_animcorp_terrestrial_prevalence.png", width = 17, height = 5)
...
ggsave("../figures/empo3_animcorp_riverine_prevalence.png", width = 17, height = 5)
...
ggsave("../figures/empo3_animcorp_marine_prevalence.png", width = 17, height = 5)
...
x = melt_list[["Terrestrial"]][[2]]

cutpoints <- c(2, 5, 10, 100, 1000, 10000, 200000)

table(cut(x$value, cutpoints))
