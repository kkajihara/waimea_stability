...
make_emp_hab_list <- function(hab, hab_list, meta_table, asv_table) {
  ...
  for (e in empo3) {
    ...
    asv_sums <- asv_sums[asv_sums$prev>=0.5,]
...
new_fung_abun_prevfilter_on_empo <- new_fung_abun_prevfilter_on_empo[,colSums(new_fung_abun_prevfilter_on_empo)>0] #329 OTUs, 1305 samples at 50% prevalence


saveRDS(new_fung_abun_prevfilter_on_empo, "../networks/input_data/fully_filtered_fung_otu_table.rds")
...
```{r}
ff <- readRDS("../networks/input_data/fully_filtered_fung_otu_table.rds")

colnames(ff) <- paste("X", colnames(ff), sep="")

# clr transformation with pseudo count
  #dat_clr <- log(ff+1) - rowMeans(log(ff+1))

# need rownames = false or MENAP will throw an error
write.table(ff, "../menap/fungi_prev50_for_menap.txt", sep = "\t", row.names = FALSE)
...
bact_abun_prevfilter_on_empo <- bact_abun_prevfilter_on_empo[,colSums(bact_abun_prevfilter_on_empo)>0] #3772 OTUs, 1313 samples at 50% prevalence
...
filtered_fungi <- new_fung_abun_prevfilter_on_empo  # 1305 samples
...
matched_ids <- fung_meta_match$sample_id # 1263 samples


# subset otu tables by samples matched in 16S and ITS
fung_otu_match <- new_fung_abun_prevfilter_on_empo[,colnames(new_fung_abun_prevfilter_on_empo) %in% fung_meta_match$x_seq_id] # 1263 samples remain
fung_otu_match <- fung_otu_match[rowSums(fung_otu_match)>0,] # 329 otus remain (0 removed)

bact_otu_match <- bact_abun_prevfilter_on_empo[,colnames(bact_abun_prevfilter_on_empo) %in% bact_meta_match$sequencing_id] # 1263 samples remain
bact_otu_match <- bact_otu_match[rowSums(bact_otu_match)>0,] # 3772 otus remain (0 removed)


saveRDS(fung_otu_match, "../intermediates/fully_filtered_fungal_otu_table_matched_up.rds")
saveRDS(fung_meta_match, "../intermediates/fully_filtered_fungal_otu_metadata_matched_up.rds")
saveRDS(bact_otu_match, "../intermediates/fully_filtered_bact_otu_table_matched_up.rds")
saveRDS(bact_meta_match, "../intermediates/fully_filtered_bact_otu_metadata_matched_up.rds")
...
fung_for_menap_clr <- log(fung_for_menap+0.01) - rowMeans(log(fung_for_menap+0.01))

library(tibble)

fung_for_menap <- rownames_to_column(fung_for_menap, "ID")

write.table(fung_for_menap, "../menap/fungi_prev50_otus_with_id.txt", sep = "\t", row.names = FALSE)
...
write.table(bact_for_menap, "../menap/bact_prev50_otus_with_id.txt", sep = "\t", row.names = FALSE)



# merge test - NOT REAL DATA
all <- merge(fung_for_menap, bact_for_menap, by = "row.names", all = TRUE)
all <- column_to_rownames(all, "Row.names")
all[is.na(all)] <- 0.01

write.table(all, "../menap/test_crossdom_nums.txt", sep = "\t", row.names = FALSE)
