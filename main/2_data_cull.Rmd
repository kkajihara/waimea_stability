...
ps_noncontam <- readRDS("../intermediates/decontaminated_phyloseq_object.rds")

abun <- as.data.frame(otu_table(ps_noncontam))
tax <- as.data.frame(tax_table(ps_noncontam))
its_meta <- as.data.frame(sample_data(ps_noncontam))
...
culled_abun <- abun[,colSums(abun)>160] # 60 samples removed, 1614 remain
# remove ASVs with only one read
culled_abun <- culled_abun[rowSums(culled_abun)>1,] # 13 ASVs removed, 52057 remain

saveRDS(culled_abun, "../intermediates/culled_asv_table.rds")


# cull tax table
culled_tax <- tax[tax$OTU %in% rownames(culled_abun),]
