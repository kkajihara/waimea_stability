...
new_fung_abun_prevfilter_on_empo <- new_fung_abun_prevfilter_on_empo[,colSums(new_fung_abun_prevfilter_on_empo)>0] # 2140 OTUs, 1316 samples at 20% prevalence


saveRDS(new_fung_abun_prevfilter_on_empo, "../networks/input_data/fung_prev20_otus_matched_up.rds")
...
bact_abun_prevfilter_on_empo <- bact_abun_prevfilter_on_empo[,colSums(bact_abun_prevfilter_on_empo)>0] #12105 OTUs, 1313 samples at 20% prevalence


filt_bact_meta <- new_bact_meta[new_bact_meta$sequencing_id %in% names(bact_abun_prevfilter_on_empo),]


saveRDS(bact_abun_prevfilter_on_empo, "../networks/input_data/bact_prev20_otus_matched_up.rds")
...
filtered_fungi <- new_fung_abun_prevfilter_on_empo  # 1316 samples
...
matched_ids <- fung_meta_match$sample_id # 1273 samples


# subset otu tables by samples matched in 16S and ITS
fung_otu_match <- new_fung_abun_prevfilter_on_empo[,colnames(new_fung_abun_prevfilter_on_empo) %in% fung_meta_match$x_seq_id] # 1273 samples remain
fung_otu_match <- fung_otu_match[rowSums(fung_otu_match)>0,] # 2140 otus remain (0 removed)

bact_otu_match <- bact_abun_prevfilter_on_empo[,colnames(bact_abun_prevfilter_on_empo) %in% bact_meta_match$sequencing_id] # 1273 samples remain
bact_otu_match <- bact_otu_match[rowSums(bact_otu_match)>0,] # 12105 otus remain (0 removed)


saveRDS(fung_otu_match, "../intermediates/prev20_fungal_otu_table_matched_up.rds")
saveRDS(fung_meta_match, "../intermediates/prev20_fungal_otu_metadata_matched_up.rds")
saveRDS(bact_otu_match, "../intermediates/prev20_bact_otu_table_matched_up.rds")
saveRDS(bact_meta_match, "../intermediates/prev20_bact_otu_metadata_matched_up.rds")
...
write.table(fung_for_menap, "../menap/fungi_prev20_otus_with_id.txt", sep = "\t", row.names = FALSE)
...
write.table(bact_for_menap, "../menap/bact_prev20_otus_with_id.txt", sep = "\t", row.names = FALSE)
