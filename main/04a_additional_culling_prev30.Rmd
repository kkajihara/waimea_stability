...
make_emp_hab_list <- function(hab, hab_list, meta_table, asv_table) {
  ...
  for (e in empo3) {
    ...
    asv_sums <- asv_sums[asv_sums$prev>=0.3,]
...
new_fung_abun_prevfilter_on_empo <- new_fung_abun_prevfilter_on_empo[,colSums(new_fung_abun_prevfilter_on_empo)>0] # 1088 OTUs, 1311 samples at 20% prevalence


saveRDS(new_fung_abun_prevfilter_on_empo, "../networks/input_data/fung_prev30_otus_matched_up.rds")
...
bact_abun_prevfilter_on_empo <- bact_abun_prevfilter_on_empo[,colSums(bact_abun_prevfilter_on_empo)>0] #7729 OTUs, 1313 samples at 20% prevalence


filt_bact_meta <- new_bact_meta[new_bact_meta$sequencing_id %in% names(bact_abun_prevfilter_on_empo),]


saveRDS(bact_abun_prevfilter_on_empo, "../networks/input_data/bact_prev30_otus_matched_up.rds")
...
filtered_fungi <- new_fung_abun_prevfilter_on_empo  # 1311 samples
...
matched_ids <- fung_meta_match$sample_id # 1268 samples
...
fung_otu_match <- fung_otu_match[rowSums(fung_otu_match)>0,] # 1088 otus remain (0 removed)
...
bact_otu_match <- bact_otu_match[rowSums(bact_otu_match)>0,] # 7729 otus remain (0 removed)


saveRDS(fung_otu_match, "../intermediates/prev30_fungal_otu_table_matched_up.rds")
saveRDS(fung_meta_match, "../intermediates/prev30_fungal_otu_metadata_matched_up.rds")
saveRDS(bact_otu_match, "../intermediates/prev30_bact_otu_table_matched_up.rds")
saveRDS(bact_meta_match, "../intermediates/prev30_bact_otu_metadata_matched_up.rds")
...
write.table(fung_for_menap, "../menap/fungi_prev30_otus_with_id.txt", sep = "\t", row.names = FALSE)
...
write.table(bact_for_menap, "../menap/bact_prev30_otus_with_id.txt", sep = "\t", row.names = FALSE)
