...
Goal: Want to see if hub taxa significantly associate with certain empo3, trophic, or habitats.
...
test <- fung_abun[rownames(fung_abun)==fung_hubs_only$name[1],]
test <- test[,colSums(test)>0]

test2 <- fung_abun[rownames(fung_abun)==fung_hubs_only$name[2],]
test2 <- test2[,colSums(test2)>0]


hmm <- data.frame(rep(rownames(test), 6),
                  names(test),
                  fung_meta$empo_3[match(names(test), fung_meta$x_seq_id)],
                  fung_hub_data$connectivity[match(rownames(test), fung_hub_data$name)])
names(hmm) <- c("OTU", "Sample", "empo_3", "z_score")


hmm2 <- data.frame(rep(rownames(test2), length(test2)),
                  names(test2),
                  fung_meta$empo_3[match(names(test2), fung_meta$x_seq_id)],
                  fung_hub_data$connectivity[match(rownames(test2), fung_hub_data$name)])
names(hmm2) <- c("OTU", "Sample", "empo_3", "z_score")

test <- rbind(hmm, hmm2)

# Load the required library (e.g., "lme4" for mixed-effects models)
library(lme4)

# Perform the nested ANOVA
nested_anova <- lmer(z_score ~ empo_3 + (1 | Sample/OTU), data = test)
