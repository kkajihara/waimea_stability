...
fung_dat <- lapply(fung_gradient_networks, get_distribution_data)
...
fung_relabun <- lapply(fung_data, get_relabun)

# make data frame with proportion and prevalence and other data for each plot
for (i in 1:length(fung_dat)) {
  V(fung_gradient_networks[[i]])$proportion <- apply(fung_dat[[i]], 1, function(c) sum(c!=0) / dim(fung_dat[[i]])[2])
  V(fung_gradient_networks[[i]])$relabun <- apply(fung_relabun[[i]], 1, function(b) sum(b) / dim(fung_dat[[i]])[2])
  
  fung_dat[[i]]$pct_samples <- V(fung_gradient_networks[[i]])$proportion[match(V(fung_gradient_networks[[i]])$name, fung_dat[[i]]$OTU)]
  fung_dat[[i]]$relabun <- V(fung_gradient_networks[[i]])$relabun[match(V(fung_gradient_networks[[i]])$name, fung_dat[[i]]$OTU)]
...
for (i in 1:7) {
  ...
  fung_key_plots[[i]] <- ggplot(fung_dat[[i]], aes(x=degree, y=btwn_cent)) +
    ...
    labs(color="Type", size="Prevalence", title=paste("ITS Keystone Candidates, Plot:", plots[[i]]))
...
bact_dat <- lapply(bact_gradient_networks, get_distribution_data)

bact_relabun <- lapply(bact_data, get_relabun)

# make data frame with proportion and prevalence and other data for each plot
for (i in 1:length(bact_dat)) {
  V(bact_gradient_networks[[i]])$proportion <- apply(bact_dat[[i]], 1, function(c) sum(c!=0) / dim(bact_dat[[i]])[2])
  V(bact_gradient_networks[[i]])$relabun <- apply(bact_relabun[[i]], 1, function(b) sum(b) / dim(bact_dat[[i]])[2])
  
  bact_dat[[i]]$pct_samples <- V(bact_gradient_networks[[i]])$proportion[match(V(bact_gradient_networks[[i]])$name, bact_dat[[i]]$OTU)]
  bact_dat[[i]]$relabun <- V(bact_gradient_networks[[i]])$relabun[match(V(bact_gradient_networks[[i]])$name, bact_dat[[i]]$OTU)]
...
cross_dat <- lapply(cross_gradient_networks, get_distribution_data)
...
cross_relabun <- lapply(cross_data, get_relabun)

# make data frame with proportion and prevalence and other data for each plot
for (i in 1:length(cross_dat)) {
  V(cross_gradient_networks[[i]])$proportion <- apply(cross_dat[[i]], 1, function(c) sum(c!=0) / dim(cross_dat[[i]])[2])
  V(cross_gradient_networks[[i]])$relabun <- apply(cross_relabun[[i]], 1, function(b) sum(b) / dim(cross_dat[[i]])[2])
  
  cross_dat[[i]]$pct_samples <- V(cross_gradient_networks[[i]])$proportion[match(V(cross_gradient_networks[[i]])$name, cross_dat[[i]]$OTU)]
  cross_dat[[i]]$relabun <- V(cross_gradient_networks[[i]])$relabun[match(V(cross_gradient_networks[[i]])$name, cross_dat[[i]]$OTU)]
...
saveRDS(keystone_data, "../../intermediates/gradient/terrestrial/gradient_keystone_data.rds")
```
...
fung_key_plots[[1]] <- ggplot(fung_dat[[1]], aes(x=degree, y=btwn_cent)) +
    ...
    theme(legend.key.size = unit(.4, "cm"))+
    scale_color_manual(values=c("dodgerblue", "black")) +
    labs(color="Type", size="Prevalence", title=paste("ITS Keystone Candidates, Plot:", plots[[1]]))
...
fung_key_plots[[2]] <- ggplot(fung_dat[[2]], aes(x=degree, y=btwn_cent)) +
    ...
    theme(legend.key.size = unit(.4, "cm"))+
    scale_color_manual(values=c("dodgerblue", "black")) +
    labs(color="Type", size="Prevalence", title=paste("ITS Keystone Candidates, Plot:", plots[[2]]))
...
fung_key_plots[[3]] <- ggplot(fung_dat[[3]], aes(x=degree, y=btwn_cent)) +
    ...
    theme(legend.key.size = unit(.4, "cm"))+
    scale_color_manual(values=c("dodgerblue", "black")) +
    labs(color="Type", size="Prevalence", title=paste("ITS Keystone Candidates, Plot:", plots[[3]]))


fung_key_plots[[4]] <- ggplot(fung_dat[[4]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(fung_both_outliers[[4]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
    ...
    theme(legend.key.size = unit(.4, "cm"))+
    scale_color_manual(values=c("dodgerblue", "black")) +
    labs(color="Type", size="Prevalence", title=paste("ITS Keystone Candidates, Plot:", plots[[4]]))


fung_key_plots[[5]] <- ggplot(fung_dat[[5]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(fung_both_outliers[[5]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
    ...
    theme(legend.key.size = unit(.4, "cm"))+
    scale_color_manual(values=c("dodgerblue", "black")) +
    labs(color="Type", size="Prevalence", title=paste("ITS Keystone Candidates, Plot:", plots[[5]]))


fung_key_plots[[6]] <- ggplot(fung_dat[[6]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(fung_both_outliers[[6]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
    ...
    theme(legend.key.size = unit(.4, "cm"))+
    scale_color_manual(values=c("dodgerblue", "black")) +
    labs(color="Type", size="Prevalence", title=paste("ITS Keystone Candidates, Plot:", plots[[6]]))


fung_key_plots[[7]] <- ggplot(fung_dat[[7]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(fung_both_outliers[[7]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
    ...
    theme(legend.key.size = unit(.4, "cm"))+
    scale_color_manual(values=c("dodgerblue", "black")) +
    labs(color="Type", size="Prevalence", title=paste("ITS Keystone Candidates, Plot:", plots[[7]]))

names(fung_key_plots) <- plots

fung_key_plots <- fung_key_plots[c("Beach", "Estuary", "Entrance", "Confluence", "Waterfall", "DrumRoad", "Ridge")]

all_fung <- ggarrange(plotlist = fung_key_plots, ncol = 4, nrow = 2, common.legend = TRUE, legend = "right") + bgcolor("white")

ggsave("../../figures/gradient/terrestrial/fung_keystone_gradient.jpg", width = 25, height=10)
...
bact_key_plots[[4]] <- ggplot(bact_dat[[4]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(bact_both_outliers[[4]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
...
bact_key_plots[[5]] <- ggplot(bact_dat[[5]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(bact_both_outliers[[5]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
...
bact_key_plots[[6]] <- ggplot(bact_dat[[6]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(bact_both_outliers[[6]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
...
bact_key_plots[[7]] <- ggplot(bact_dat[[7]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(bact_both_outliers[[7]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
    ...
    theme(legend.key.size = unit(.4, "cm"))+
    scale_color_manual(values=c("dodgerblue", "black")) +
    labs(color="Type", size="Prevalence", title=paste("16S Keystone Candidates, Plot:", plots[[7]]))


names(bact_key_plots) <- plots

bact_key_plots <- bact_key_plots[c("Beach", "Estuary", "Entrance", "Confluence", "Waterfall", "DrumRoad", "Ridge")]

all_bact <- ggarrange(plotlist = bact_key_plots, ncol = 4, nrow = 2, common.legend = TRUE, legend = "right") + bgcolor("white")

ggsave("../../figures/gradient/terrestrial/bact_keystone_gradient.jpg", width = 25, height=10)
...
cross_key_plots[[4]] <- ggplot(cross_dat[[4]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(cross_both_outliers[[4]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
...
cross_key_plots[[5]] <- ggplot(cross_dat[[5]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(cross_both_outliers[[5]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
...
cross_key_plots[[6]] <- ggplot(cross_dat[[6]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(cross_both_outliers[[6]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
...
cross_key_plots[[7]] <- ggplot(cross_dat[[7]], aes(x=degree, y=btwn_cent)) +
    geom_point(aes(size = prevalence,
                   color = ifelse(OTU %in% rownames(cross_both_outliers[[7]]), "Keystone Candidate", "Other")),
               position = "jitter") +
    ylab("Betweenness Centrality") + 
    xlab("Node degree") +
    scale_y_continuous(trans=log10_trans(),breaks=trans_breaks("log10",function(x)10^x),
    ...
    theme(legend.key.size = unit(.4, "cm"))+
    scale_color_manual(values=c("dodgerblue", "black")) +
    labs(color="Type", size="Prevalence", title=paste("Cross Keystone Candidates, Plot:", plots[[7]]))


names(cross_key_plots) <- plots

cross_key_plots <- cross_key_plots[c("Beach", "Estuary", "Entrance", "Confluence", "Waterfall", "DrumRoad", "Ridge")]

all_cross <- ggarrange(plotlist = cross_key_plots, ncol = 4, nrow = 2, common.legend = TRUE, legend = "right") + bgcolor("white")

ggsave("../../figures/gradient/terrestrial/cross_keystone_gradient.jpg", width = 25, height=10)
