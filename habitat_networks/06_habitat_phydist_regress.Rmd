...
test_pl <- ggplot(dat_for_gg, aes(y=log_val, x = mpd)) +
  geom_point() +
  ggtitle("Habitat Sample Richness vs MPD") +
  #geom_smooth(method = "lm", se = FALSE) +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
        panel.background=element_blank()) +
  scale_y_continuous(breaks = seq(8, 14, 2), limits = c(c(8, 14))) +
  labs(subtitle = paste("Linear regression results:", "R2 =", round(summary(test_lm)$r.squared, 3), "P < 0.001"), x = "Mean Phylogenetic Distance Effect Scores", y = "log(Richness by Sample)")
