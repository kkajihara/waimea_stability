---
title: "10_hostnon_keystone_knockout_robustness"
...
# keystone data
keystone_data <- readRDS("../intermediates/hostnon/hostnon_keystone_data.rds")
...
for (i in 1:length(fung_keystone_data)) {
  net_no_key <- delete_vertices(fung_hostnon_networks[[i]], fung_keystone_data[[i]]$OTU)
...
for (i in 1:length(fung_keystone_data)) {
  bc_robust <- robustness(fung_hostnon_networks[[i]], type = "vertex", "btwn.cent")
  bc_robust$Type <- "Full"
...
for (i in 1:length(fung_keystone_data)) {
  fung_key_rob_plots[[i]] <- ggplot(fung_robustness_data[[i]], aes(x=removed.pct, y=comp.pct, group=Type, color=Type)) +
    labs(x="Percentage of nodes removed", y="Size of largest connected component",
       title=paste0(empo1[[i]], ", ", nrow(fung_keystone_data[[i]]), " Keystone OTUs")) +
    geom_line(size=1.5) +
...
names(fung_key_rob_plots) <- empo1
#fung_key_rob_plots <- fung_key_rob_plots[c("Marine", "Riverine", "Terrestrial")]


all_fung <- fung_key_rob_plots[[1]] + fung_key_rob_plots[[2]]  & theme(legend.position = "bottom")
#plots <- plots & xlab(NULL) & ylab(NULL)
all_fung <- all_fung + plot_layout(guides = "collect") 

ggsave("../figures/finals/hostnon/fung_keystone_removal_robustness_by_hostnon_for_affinity.pdf", all_fung, width=12, height=6.25)
ggsave("../figures/finals/hostnon/fung_keystone_removal_robustness_by_hostnon_for_affinity.png", all_fung, width=12, height=6.25)
...
for (i in 1:length(bact_keystone_data)) {
  net_no_key <- delete_vertices(bact_hostnon_networks[[i]], bact_keystone_data[[i]]$OTU)
...
for (i in 1:length(bact_keystone_data)) {
  bc_robust <- robustness(bact_hostnon_networks[[i]], type = "vertex", "btwn.cent")
  bc_robust$Type <- "Full"
...
for (i in 1:length(bact_keystone_data)) {
  bact_key_rob_plots[[i]] <- ggplot(bact_robustness_data[[i]], aes(x=removed.pct, y=comp.pct, group=Type, color=Type)) +
    labs(x="Percentage of nodes removed", y="Size of largest connected component",
       title=paste0(empo1[[i]], ", ", nrow(bact_keystone_data[[i]]), " Keystone OTUs")) +
    geom_line(size=1.5) +
  theme(legend.key = element_rect(fill="white")) +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
...
# ggsave("../figures/hostnon/one_bact_keystone_removal_rob_plot.png", bact_key_rob_plots[[1]], width=10, height=6)


#sort plots by plot order
names(bact_key_rob_plots) <- empo1
#bact_key_rob_plots <- bact_key_rob_plots[c("Marine", "Riverine", "Terrestrial")]


all_bact <- bact_key_rob_plots[[1]] + bact_key_rob_plots[[2]] & theme(legend.position = "bottom")
#plots <- plots & xlab(NULL) & ylab(NULL)
all_bact <- all_bact + plot_layout(guides = "collect") 

ggsave("../figures/finals/hostnon/bact_keystone_removal_robustness_by_hostnon_for_affinity.pdf", all_bact, width=12, height=6.25)

ggsave("../figures/finals/hostnon/bact_keystone_removal_robustness_by_hostnon_for_affinity.png", all_bact, width=12, height=6.25)
...
for (i in 1:length(cross_keystone_data)) {
  net_no_key <- delete_vertices(cross_hostnon_networks[[i]], cross_keystone_data[[i]]$OTU)
...
for (i in 1:length(cross_keystone_data)) {
  bc_robust <- robustness(cross_hostnon_networks[[i]], type = "vertex", "btwn.cent")
  bc_robust$Type <- "Full"
...
for (i in 1:length(cross_keystone_data)) {
  cross_key_rob_plots[[i]] <- ggplot(cross_robustness_data[[i]], aes(x=removed.pct, y=comp.pct, group=Type, color=Type)) +
    labs(x="Percentage of nodes removed", y="Size of largest connected component",
       title=paste0(empo1[[i]], ", ", nrow(cross_keystone_data[[i]]), " Keystone OTUs")) +
    geom_line(size=1.5) +
  theme(legend.key = element_rect(fill="white")) +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
...
#sort plots by plot order
names(cross_key_rob_plots) <- empo1
#cross_key_rob_plots <- cross_key_rob_plots[c("Marine", "Riverine", "Terrestrial")]

library(patchwork)

all_cross <- cross_key_rob_plots[[1]] + cross_key_rob_plots[[2]] & theme(legend.position = "bottom")
#plots <- plots & xlab(NULL) & ylab(NULL)
all_cross <- all_cross + plot_layout(guides = "collect") 


ggsave("../figures/finals/hostnon/cross_keystone_removal_robustness_by_hostnon_for_affinity.pdf", all_cross, width=12, height=6.25)

ggsave("../figures/finals/hostnon/cross_keystone_removal_robustness_by_hostnon_for_affinity.png", all_cross, width=12, height=6.25)
...
```{r}
saveRDS(list(fung_bc_rob_nokey, bact_bc_rob_nokey, cross_bc_rob_nokey), "../intermediates/nokey_robustness_hostnon_data.rds")
```
