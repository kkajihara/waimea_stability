...
run_se <- function(otu_table) {
  ...
  net <- spiec.easi(tab, method='mb', lambda.min.ratio=1e-3, nlambda = 50, scr=TRUE)
}


fung_gradient_networks <- lapply(fung_data, run_se)
saveRDS(fung_gradient_networks, "~/cmaiki_lts/kaciekaj/waimea/networks/prev20/gradient/outputs/six_empos_fung_gradient_networks_nlambda50.rds")


bact_gradient_networks <- lapply(bact_data, run_se)
saveRDS(bact_gradient_networks, "~/cmaiki_lts/kaciekaj/waimea/networks/prev20/gradient/outputs/six_empos_bact_gradient_networks_nlambda50.rds")
...
saveRDS(cross_gradient_networks, "~/cmaiki_lts/kaciekaj/waimea/networks/prev20/gradient/outputs/six_empos_cross_gradient_networks_nlambda50.rds")
