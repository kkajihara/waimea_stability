...
run_se <- function(otu_table) {
  ...
  bargs <- list(thresh=0.05)
  net <- spiec.easi(tab, method='mb', pulsar.params=bargs, scr=TRUE)
}
...
saveRDS(bact_gradient_networks, "~/cmaiki_lts/kaciekaj/waimea/networks/prev20/gradient/outputs/riverine/riverine_bact_gradient_network_thresh02.rds")
# 
# 
# 
# ######## cross domain
