...
run_se <- function(otu_table) {
  ...
  net <- spiec.easi(tab, method='mb', lambda.min.ratio=1e-3, nlambda=200, pulsar.params=bargs, scr=TRUE)
}
...
saveRDS(bact_gradient_networks, "~/cmaiki_lts/kaciekaj/waimea/networks/prev20/gradient/outputs/six_empos_bact_gradient_networks_nlambda200.rds")
# 
# 
# 
# ######## cross domain
