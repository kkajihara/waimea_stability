...
for (i in 1:length(fung_data)) {
  ...
  bargs <- list(thresh=0.01)
  net <- multi.spiec.easi(list(f_abun, b_abun), method='mb', lambda.min.ratio=1e-3, nlambda=200, pulsar.params = bargs, scr=TRUE)

  cross_gradient_networks[[i]] <- net
}


saveRDS(cross_gradient_networks, "~/cmaiki_lts/kaciekaj/waimea/networks/prev20/gradient/outputs/six_empos_cross_gradient_networks_nlambda200.rds")
