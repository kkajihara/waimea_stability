...
bargs <- list(conffile=".batchtools.conf.R")
crossdom_net <- multi.spiec.easi(list(f_abun, b_abun), method='mb', lambda.min.ratio=1e-2, nlambda=20, scr=TRUE,
                             pulsar.select='batch', pulsar.params=bargs)

saveRDS(crossdom_net, "~/cmaiki_lts/kaciekaj/waimea/networks/prev50_finals/outputs/crossdom_mb_nlambda20.rds")
